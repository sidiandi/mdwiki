  ...
  httpRequest.get({url: url, headers: { 'user-agent': 'mdwiki' }}, function (error, response, body) {
    if (error) {
      ...
      if (response.headers !== undefined) {
        logger.info('Response from github request %s: statusCode=%s, x-ratelimit-limit: %s, x-ratelimit-remaining: %s ',
                    url, response.statusCode, response.headers['x-ratelimit-limit'],
