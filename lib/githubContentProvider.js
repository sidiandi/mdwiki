...
GitHubContentProvider.prototype.createOrUpdatePage = function (pageName, markdownText, messageToGithub) {
  var deferred = q.defer(),
  ...
  httpRequest.put({url: url, headers: { 'user-agent': 'mdwiki' }, body: messageToGithub, json: true}, function (error, response, body) {
    if (error) {
      ...
      error = new Error('Unexpected response from github: %s', response.body.message);
...
GitHubContentProvider.prototype.createPage = function (commitMessage, pageName, markdownText) {
  var encodedContent = new Buffer(markdownText).toString('base64');
  ...
  return this.createOrUpdatePage(pageName, markdownText, messageBody);
};
...
GitHubContentProvider.prototype.updatePage = function (commitMessage, pageName, markdownText) {
  ...
  this.getPage(pageName).then(function (page) {
    ...
    this.createOrUpdatePage.call(this, pageName, markdownText, messageBody)
      .then(deferred.resolve)
      .catch(deferred.reject);
...
GitHubContentProvider.prototype.buildPageMessage = function (commitMessage, content, pageSha) {
  var message = {
    message: commitMessage,
