...
GitHubContentProvider.prototype.updatePage = function (session, commitMessage, page) {
  var deferred = q.defer();

  if (session === null || session.oauth === undefined) {
    var error = new Error('not authenticated');
    error.status = 401;
    deferred.reject(error);
  } else {
    var url = urlBuilder.buildPageUpdateUrl(this.userName, this.repositoryName, page.name, session.oauth);

    var messageBody = {
      message: commitMessage,
      content: page.content,
      sha: sha1(page.content)
    };

    httpRequest.put({url: url, body: messageBody}, function (error, response, body) {
      if (error) {
        deferred.reject(error);
        return;
      } else if (response.statusCode !== 200 && response.statusCode !== 201) {
        error = new Error('Unexpected response from github');
